...
from src.agents.quality_issue_analysis_agent import QualityIssueAnalysisAgent
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Compare quality issue analysis cases across LLM providers."
    )
    parser.add_argument(
        "--providers",
    ...
    parser.add_argument(
        "--cases",
        type=lambda value: parse_csv_list(
            value, tuple(QUALITY_ISSUE_CASES), "cases"
        ),
        default=list(QUALITY_ISSUE_CASES),
    )
    parser.add_argument(
        "--run-log",
        type=Path,
